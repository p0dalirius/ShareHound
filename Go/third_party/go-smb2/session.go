...
func sessionSetup(conn *conn, i Initiator, ctx context.Context) (*session, error) {
	...
	preauthIntegrityHashValue := conn.preauthIntegrityHashValue
	updatePreauthIntegrityHash := func(pkt []byte) error {
		...
		case smb2.SHA512:
			h := sha512.New()
			h.Write(preauthIntegrityHashValue[:])
			h.Write(pkt)
			h.Sum(preauthIntegrityHashValue[:0])
			return nil
		default:
			return &InvalidResponseError{"unknown hash algorithm"}
		}
	}
	...
	var s *session
	var r smb2.SessionSetupResponseDecoder
	...
	for {
		...
		if err := updatePreauthIntegrityHash(rr.pkt); err != nil {
			return nil, err
		}
		...
		securityBuffer := r.SecurityBuffer()
		if len(securityBuffer) != 0 {
			outputToken, err = spnego.AcceptSecContext(securityBuffer)
			if err != nil {
				return nil, &InvalidResponseError{err.Error()}
			}
		}
	...
	s.preauthIntegrityHashValue = preauthIntegrityHashValue

	if s.sessionFlags&(smb2.SMB2_SESSION_FLAG_IS_GUEST|smb2.SMB2_SESSION_FLAG_IS_NULL) == 0 {
		if err := s.initSecurity(spnego.SessionKey()); err != nil {
			return nil, err
		}
	}
...
func (s *session) initSecurity(sessionKey []byte) error {
